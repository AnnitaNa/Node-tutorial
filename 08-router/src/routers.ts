...
function readAndParse(req: Request, res: Response, next: NextFunction) {
	const read: string = fs.readFileSync(jsonPath, {encoding: "utf-8"});
...
router.get("/", (req: Request, res: Response) => {  //  '/' says it is the same route used in main.js //! app.use('/product', router)
...
router.put("/:id", (req: Request, res: Response) => {
	...
	const newData: Iproducts[] = data.map((product: Iproducts) => {
